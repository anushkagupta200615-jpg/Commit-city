...
export function buildCityModel(data) {
  ...
  // ---- city permits: license coverage across own repos ----
  const licCounts = new Map();
  const unlicensedNames = [];
  for (const r of ownRepos) {
    if (r.license) licCounts.set(r.license, (licCounts.get(r.license) || 0) + 1);
    else unlicensedNames.push(r.name);
  }
  const licenses = {
    breakdown: [...licCounts.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([name, count]) => ({ name, count })),
    unlicensed: unlicensedNames.length,
    unlicensedSample: unlicensedNames.slice(0, 4),
  };

  // ---- top topics across own repos (for smarter issue matching) ----
